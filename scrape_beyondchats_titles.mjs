...
async function scrapeBeyondChats() {
  const browser = await puppeteer.launch({
    headless: true,
    executablePath: 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe', // Adjust Chrome path
  ...
  const articles = await page.evaluate(() => {
    return Array.from(document.querySelectorAll('.blog__card h2 a')).map(a => a.innerText.trim());
  });
